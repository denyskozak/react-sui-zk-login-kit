...
export const ZkLogin = (props) => {
    const { providers, observeTokenInURL = true, proverProvider, userSalt, title = 'Sign In With', subTitle = 'Your Preferred Service' } = props;
    ...
    const handleGoogleLogin = () => {
        if (providers.google && nonce) {
            handleRedirectToGoogle(providers.google.clientId, providers.google.redirectURI, nonce);
        }
    };
    const handleTwitchLogin = () => {
        if (providers.twitch && nonce) {
            handleRedirectToTwitch(providers.twitch.clientId, providers.twitch.redirectURI, nonce);
        }
    };
    const renderProvider = {
        google: (_jsx(Icon, { onClick: handleGoogleLogin, children: _jsx(IconImg, { src: GoogleIcon }) }, "google")),
        twitch: (_jsx(Icon, { onClick: handleTwitchLogin, children: _jsx(TwitchIconImg, { src: TwitchIcon }) }, "twitch"))
    };
    return (_jsxs(Container, { children: [!zkLoginAddress && (_jsxs(_Fragment, { children: [_jsx(Typography, { children: title }), _jsx(Typography, { children: subTitle })] })), zkProofLoading && (_jsx(Typography, { children: "Loading ZK Proof ..." })), zkLoginAddress && (_jsxs(_Fragment, { children: [_jsxs(Typography, { children: ["Your address: ", _jsx(Code, { children: zkLoginAddress })] }), _jsx(Button, { onClick: () => navigator.clipboard.writeText(zkLoginAddress), children: "Copy Address" })] })), !zkLoginAddress && !zkProofLoading && (_jsx(IconContainer, { children: providerList.map(([providerName]) => renderProvider[providerName]) }))] }));
};
